...
plot(age, glucose_conc, main = "Plasma glucose concentration on age", xlab = "Age", ylab = "Glucose",  col = ifelse(diabetes == 1, "green", "red"),  # Color by diabetes levels
     pch = 19)
#Step2

library(dplyr)
train=data.frame(glucose_conc, age, diabetes) #Selects the columns glucose concentration, age and diabetes

m1 = glm(diabetes~age+glucose_conc, family = "binomial", data = train) #Train the logistic regression model
summary(m1)
...
mc_error = sum(diag(train_confusion_matrix)) / length(pred) #The incorrect classifications are in the diagonal of the confusion matrix. Length pred represents the total no of predictions made
mc_error

plot(train$age,train$glucose_conc, main="predicted plasma glucose concentration on age",
     xlab="age", ylab="plasma glucose concentration", col = ifelse(pred == "diabetes", "green", "red"), pch=19)

#Step 3
#See page 23 of lecture 1d
#Decision boundary is a curve that seprates different classes (Diabetes or no diabetes)
#For binary model it would be when P(y=1|X) = 0.5 (Probabilistic equation)
